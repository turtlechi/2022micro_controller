...
int main(){
	...
	if(init_led(LED_gpioa, LED1_pin) != 0 || init_led(LED_gpiob, LED2_pin) != 0 || init_led(LED_gpiob, LED3_pin) != 0 || init_led(LED_gpiob, LED4_pin) != 0||init_button(BUTTON_gpio,BUTTON_pin)!=0){
		// Fail to init LED
		return -1;}
	...
	while(1){
		...
		if(state==1){
			...
			for(int a=0;a<4;a++)
			{
				...
				{
					if((led_data>>(a))&0x1){
						reset_gpio(LED_gpioa, leds[a]);
					}
					else{
						set_gpio(LED_gpioa, leds[a]);
					}
				}
				else{
					if((led_data>>(a))&0x1){
						reset_gpio(LED_gpiob, leds[a]);
					}
					else{
						set_gpio(LED_gpiob, leds[a]);
					}
				}
			}

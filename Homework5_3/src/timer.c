...
int init_sound(GPIO_TypeDef* gpio, int sou_pin){
		...
		gpio->MODER &= ~(0b11 << (2*sou_pin));
		gpio->MODER |= (0b01 << (2*sou_pin));


		// Close display test
		send_sound(gpio, sou_pin);

		return 0;

}

void send_sound(GPIO_TypeDef* gpio, int s){
		gpio->BSRR |= (1 << s);
	}
